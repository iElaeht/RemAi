  ...
  return (
    <div className="px-6 py-12 md:py-20 animate-in fade-in zoom-in duration-700">
      <div className="container mx-auto max-w-5xl">
        ...
        {/* Carta Minimalista con efecto de cristal (Glassmorphism) */}
        <div className="relative overflow-hidden bg-[#1e293b]/40 backdrop-blur-xl p-8 md:p-12 rounded-3xl border border-white/10 shadow-[0_8px_32px_0_rgba(0,0,0,0.3)]">
          ...
          {/* Fondo dinámico sutil (un pequeño gradiente de luz) */}
          ...
          <div className="relative flex flex-col md:flex-row items-center md:items-start gap-10">
            ...
            {/* Portada con sombra pronunciada */}
            ...
            <div className="flex flex-col items-center md:items-start gap-4 w-full text-center md:text-left">
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-2">
                {manga.tags?.slice(0, 4).map((tag: string) => (
                  <span key={tag} className="text-[10px] uppercase font-bold px-3 py-1 bg-white/5 border border-white/10 rounded-full text-blue-200">
                    {tag}
                  </span>
                ))}
              </div>

              <p className="text-gray-300 text-sm md:text-base leading-relaxed text-justify mt-2 max-w-2xl font-light">
                {manga.description}
              </p>
              
              {/* Botones de acción con contraste dinámico */}
              <div className="flex items-center gap-3 mt-4">
                ...
                <button className="flex items-center gap-2 bg-pink-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-pink-500 transition-all shadow-[0_0_20px_-5px_rgba(219,39,119,0.5)] active:scale-95">
                  <BookOpen size={18} /> LEER AHORA
                </button>
              </div>
            </div>
          </div>
