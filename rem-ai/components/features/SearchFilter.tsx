...
export default function SearchFilter({
  ...
  return (
    <div ref={containerRef} className="max-w-2xl mx-auto mt-8 mb-12">
      <div
        className={`bg-[#111827] rounded-2xl border border-white/10 shadow-2xl overflow-hidden transition-all ${isFilterOpen ? "ring-1 ring-pink-500/50" : ""}`}
      >
        <div className="flex items-center p-2">
          ...
          {/* Botón de Buscar: Solo habilitado si hay texto */}
          <button
            onClick={handleSearchClick}
            disabled={!searchQuery}
            className={`p-2 mr-1 transition-all rounded-lg ${searchQuery ? "text-neutral-300 hover:text-white hover:bg-white/5" : "text-neutral-700 cursor-default"}`}
            title="Buscar"
          >
            <ArrowRight size={18} />
          </button>

          {/* Botón de borrar (X): Comportamiento igual */}
          <button
            onClick={() => setSearchQuery("")}
            className={`p-2 transition-colors ${searchQuery ? "text-neutral-300 hover:text-white" : "text-neutral-700"}`}
          >
            <X size={18} />
          </button>
                  ...
                  <div className="p-4 pt-0 grid grid-cols-2 md:grid-cols-3 gap-2">
                    {Object.entries(tags).map(([name, id]) => (
                      <button
                        ...
                        className={`text-xs p-2 rounded-lg transition-all text-left truncate ${pathname.includes(id) ? "bg-pink-500 text-white font-bold" : "text-neutral-400 hover:bg-white/5 hover:text-white"}`}
                      >
                        {name}
                      </button>
                    ))}
