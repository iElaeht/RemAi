...
export default function InfiniteCarousel({ mangas }: { mangas: MangaResponse[] }) {
  return (
    <div className="relative flex overflow-hidden mask-fade px-6 md:px-24">
      ...
      >
        {[...mangas, ...mangas].map((manga, index) => (
          <div key={`${manga.id}-${index}`} className="w-[200px] shrink-0" onContextMenu={(e) => e.preventDefault()}>
