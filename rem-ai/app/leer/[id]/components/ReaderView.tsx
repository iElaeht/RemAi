...
export default function ReaderView({
  ...
  return (
    <div className="relative w-full min-h-screen bg-[#0a0f1a] flex flex-col select-none">
      <div
        className={`w-full h-16 flex flex-col items-center justify-center bg-[#0a0f1a]/90 backdrop-blur-sm gap-1.5 shrink-0 z-50 top-0 border-b border-white/5 transition-opacity ${isZoomed ? "opacity-0 pointer-events-none" : "opacity-100"}`}
      >
        <span className="text-gray-400 text-[10px] font-bold tracking-[0.2em] uppercase">
          Página {currentPage} / {pages?.length || 0} |{" "}
          {isZoomed ? "MODO ZOOM" : "LECTURA"}
        </span>
          ...
          return (
            <div
              ...
              className={`
                ...
                ${
                  isCurrentPageZoomed
                    ? `fixed inset-0 z-[100] bg-[#0a0f1a] w-screen h-screen cursor-default ${isTouch ? 'overflow-auto' : ''}`
            ...
            >
              <div className={`relative h-full flex items-center justify-center ${isCurrentPageZoomed ? "w-full overflow-auto" : ""}`}>
                <img
                  src={`/api/proxy/pages?url=${encodeURIComponent(`${baseUrl}/data/${hash}/${page}`)}`}
                  alt={`Página ${idx + 1}`}
                  draggable="false"
                  // Cambiamos a 'touch-action: auto' cuando está en modo zoom en móvil
                  className={`max-h-[95dvh] object-contain select-none pointer-events-none transition-transform duration-100 ease-linear ${
                    isCurrentPageZoomed && isTouch
                      ? "w-[180%] max-w-none h-auto"
                      : ""
                  }`}
                  style={{
                    // Si es Desktop, usamos tu transformación actual. Si es Mobile, dejamos que CSS controle el tamaño.
                    transform:
                      isCurrentPageZoomed && !isTouch
